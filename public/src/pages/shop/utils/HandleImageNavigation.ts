export const HandleImageNavigation = (event: Event, direction: string) => {
  ...
  if (direction === "next") {
    ...
    if (targetElement) {
      prevArrow = (targetElement.parentElement as HTMLButtonElement).children[0];
    }
  }
  if (direction === "previous") {
    nextArrow = (targetElement.parentElement as HTMLButtonElement).children[1];
    prevArrow = targetElement;
  }

  const productImagesElement = (targetElement.parentElement as HTMLButtonElement).parentElement;

  let productImagesArray = ((targetElement.parentElement as HTMLElement).parentElement as HTMLElement)
    .children;
